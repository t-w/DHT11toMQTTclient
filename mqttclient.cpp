...
mqtt_client::mqtt_client(const char *id, const char *host, int port,
                         const char *mqttUser, const char *mqttPassword) : mosquittopp(id)
{
    int keepalive = DEFAULT_KEEP_ALIVE;
    username_pw_set(mqttUser, mqttPassword);
    connect(host, port, keepalive);
}
...
void mqtt_client::on_message(const struct mosquitto_message *message)
{
    ...
    std::cout << "Got message on the topic: " << message->topic << std::endl
              << "message: " << std::string( (char *) message->payload ) << std::endl;
